...
export interface IGameData {
  ...
  lang: string
  floorY: number
  tree: { x: number; y: number; v: number; a: number }[]
  job: [number, number][]
  jobTotal: number
  countBall: number
  maxijob: number
...
const defaultGameData: IGameData = {
  ...
  totalScore: 0,
  level: 1,
  levelScore: GameOptions.minNeedValueScoreByLevel,
  bomb: 0,
  lang: GameOptions.lang,
  floorY: 0,
  tree: [],
  job: GameOptions.startJob,
  jobTotal: GameOptions.startJob.reduce((ac, el) => {
    ac = ac + el[1] * GameOptions.bodies[el[0]].score
    return ac
  }, 0),
  countBall: 0,
  maxijob: GameOptions.maxIndexFindBody
}
...
const onLoadGameData = (data: IGameData) => {
  ...
  if (!data.totalScore || isNaN(data.totalScore) || !data.job) {
    gameState.value = defaultGameData
  }
...
const onChangeLang = (langCode: string) => {
  ...
  scene.scene.get('Home').changeLocale(currentLanguage)
  scene.scene.get('Control').changeLocale(currentLanguage)
  scene.scene.get('NextLevel').changeLocale(currentLanguage)
  scene.scene.get('GameOver').changeLocale(currentLanguage)
  scene.scene.get('Message').changeLocale(currentLanguage)
